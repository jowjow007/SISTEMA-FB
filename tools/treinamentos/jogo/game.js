...
function seededRand(seed){ var s=(seed>>>0)||1; return function(){ s=(s*1103515245+12345)>>>0; return (s>>>8)/16777216; }; }
function hashStr(s){ var h=0; for(var i=0;i<s.length;i++) h=(h*31+s.charCodeAt(i))>>>0; return h||1; }
...
/* ---------------- terreno pré-renderizado (grama/pedra + decoração) ---------------- */
function prerenderTerreno(m){
  var W=m.w*TILE, H=m.h*TILE;
  var c=document.createElement('canvas'); c.width=W; c.height=H;
  var g=c.getContext('2d'); g.imageSmoothingEnabled=false;
  var rnd=seededRand(hashStr(m.nome));
  var pedra=(m.nome==='praca');
  g.fillStyle=m.base; g.fillRect(0,0,W,H);

  var nBlot=Math.round(W*H/1300);
  for (var i=0;i<nBlot;i++){
    var x=rnd()*W, y=rnd()*H, r=10+rnd()*24;
    g.globalAlpha=0.16+rnd()*0.2;
    g.fillStyle = rnd()<0.5 ? m.piso2 : shade(m.piso2, 0.86+rnd()*0.28);
    g.beginPath(); g.arc(x,y,r,0,Math.PI*2); g.fill();
  }
  g.globalAlpha=1;

  if (pedra){
    var cols=Math.ceil(W/20)+1, rows=Math.ceil(H/16)+1;
    for (var ry=0; ry<rows; ry++) for (var cxi=0; cxi<cols; cxi++){
      var lx=cxi*20+(ry%2?10:0)-10, ly=ry*16-8, f=0.86+rnd()*0.3;
      g.fillStyle=shade('#c7c0ac', f);
      rrp(g, lx, ly, 17, 13, 3); g.fill();
      g.strokeStyle='rgba(80,70,55,.30)'; g.lineWidth=1; g.stroke();
      if (rnd()<0.18){ g.fillStyle='rgba(255,255,255,.12)'; g.fillRect(lx+2, ly+2, 4,2); }
    }
  } else {
    var nSp=Math.round(W*H/55);
    for (var s=0;s<nSp;s++){
      g.globalAlpha=0.45; g.fillStyle = rnd()<0.5 ? shade(m.piso2,1.18) : shade(m.base,0.78);
      g.fillRect(rnd()*W, rnd()*H, 1, 1);
    }
    g.globalAlpha=1;
    var nTuf=Math.round(W*H/220);
    for (var t=0;t<nTuf;t++){
      var tx=rnd()*W, ty=rnd()*H;
      g.strokeStyle='rgba(28,58,16,.4)'; g.lineWidth=1;
      g.beginPath();
      g.moveTo(tx-2,ty+2); g.lineTo(tx-1,ty-3);
      g.moveTo(tx,ty+2); g.lineTo(tx+0.5,ty-4);
      g.moveTo(tx+2,ty+2); g.lineTo(tx+3,ty-2);
      g.stroke();
    }
    var florCores=['#ffffff','#ffe27a','#f2a0c4','#c9a6ea'];
    var nFlor=Math.round(W*H/850);
    for (var f2=0; f2<nFlor; f2++){
      var fx=rnd()*W, fy=rnd()*H;
      g.fillStyle='#3a7a3a'; g.fillRect(fx,fy+1,1,2);
      g.fillStyle=florCores[Math.floor(rnd()*florCores.length)];
      g.fillRect(fx-1,fy-1,3,3);
    }
    if (m.plantio){
      var pa=m.plantio, px0=pa.x0*TILE-7, py0=pa.y0*TILE-7, pw=(pa.x1-pa.x0+1)*TILE+14, ph=(pa.y1-pa.y0+1)*TILE+14;
      rrp(g,px0,py0,pw,ph,9);
      g.fillStyle='#8a6a44'; g.fill();
      g.strokeStyle='rgba(0,0,0,.18)'; g.lineWidth=3; g.stroke();
      g.strokeStyle='rgba(255,255,255,.08)'; g.lineWidth=1;
      for (var fy2=py0+8; fy2<py0+ph-4; fy2+=7){ g.beginPath(); g.moveTo(px0+5,fy2); g.lineTo(px0+pw-5,fy2); g.stroke(); }
    }
  }
  m.terrenoCV=c;
}
function desenhaAgua(x,y,t){
  var w1=Math.sin(t/450 + x*0.4)*1.1, w2=Math.cos(t/600 + y*0.3)*0.9;
  ctx.fillStyle='#2e6f97'; ctx.fillRect(x,y,TILE,TILE);
  ctx.fillStyle='#3f89b8'; ctx.fillRect(x, y+4+w1, TILE, 5);
  ctx.fillStyle='#5aa8d0'; ctx.fillRect(x, y+9+w2, TILE, 3);
  ctx.fillStyle='rgba(255,255,255,.18)'; ctx.fillRect(x, y+1, TILE, 2);
  var seed=(x*13+y*7+Math.floor(t/700))%97;
  if (seed<9){ ctx.fillStyle='rgba(255,255,255,.7)'; ctx.fillRect(x+3+(seed%6), y+3+((seed*3)%9), 2,2); }
}

/* pessoa 20x30 (pés/sombra na base). dir 0=baixo 1=cima 2=esq 3=dir. */
function drawPessoa(g, pal, dir, wf, acting){
  var pele=pal.pele||'#e8b48a', cab=pal.cabelo||'#3a2b24', roupa=pal.roupa||'#2f2440';
  var peleE=shade(pele,0.85), roupaE=shade(roupa,0.72), roupaL=shade(roupa,1.22), cabE=shade(cab,0.7);
  var pp = wf===1?1:0, ox=10; // eixo central x

  g.fillStyle='rgba(20,14,10,.28)'; g.beginPath(); g.ellipse(ox,28,7,2.4,0,0,7); g.fill();

  // pernas
  g.fillStyle='#2a2030';
  if (dir===2||dir===3){ g.fillRect(ox-4,20,4,6-pp); g.fillRect(ox+1,20,4,5+pp); }
  else { g.fillRect(ox-5,20,4,6-pp); g.fillRect(ox+2,20,4,5+pp); }
  g.fillStyle='#181018';
  g.fillRect(ox-5,25-pp,4,3); g.fillRect(ox+2,24+pp,4,3); // sapatos

  // corpo (com sombra de um lado, luz do outro)
  g.fillStyle=roupaE; g.fillRect(ox-6,11,12,11);
  g.fillStyle=roupa; g.fillRect(ox-6,11,9,11);
  g.fillStyle=roupaL; g.fillRect(ox-6,11,3,11);
  g.fillStyle='#e7b84f'; g.fillRect(ox-6,11,1,11); g.fillRect(ox+5,11,1,11); g.fillRect(ox-6,20,12,1);
  if (dir===0){ g.fillStyle='#f7f4ee'; g.fillRect(ox-2,11,5,7); g.fillStyle='#b8362e'; g.fillRect(ox-1,11,2,7); }

  // braços
  var braE = acting ? (dir===3? [ox+6,9,4,7] : dir===2? [ox-10,9,4,7] : [ox-9,9,3,6]) : [ox-9,12,3,7-pp];
  var braD = acting ? (dir===3? [ox+6,9,4,7] : dir===2? [ox-10,9,4,7] : [ox+6,9,3,6]) : [ox+6,12,3,6+pp];
  g.fillStyle=roupaE; g.fillRect(braE[0],braE[1],braE[2],braE[3]);
  g.fillStyle=roupa; g.fillRect(braD[0],braD[1],braD[2],braD[3]);
  g.fillStyle=peleE; g.fillRect(braE[0]-0.5,braE[1]+braE[3]-1,3,2);
  g.fillStyle=pele; g.fillRect(braD[0]-0.5,braD[1]+braD[3]-1,3,2);

  // cabeça (grande, estilo "chibi" acolhedor)
  g.fillStyle=peleE; g.fillRect(ox-6,2,12,10);
  g.fillStyle=pele; g.fillRect(ox-6,2,9,10);
  g.fillStyle='#f2b48c'; g.fillRect(ox-2,9,4,2); // bochecha/queixo luz
  // cabelo
  g.fillStyle=cabE; g.fillRect(ox-7,0,14,4); g.fillRect(ox-7,1,3,7); g.fillRect(ox+4,1,3,7);
  g.fillStyle=cab; g.fillRect(ox-7,0,10,4);
  if (dir===1){ g.fillStyle=cab; g.fillRect(ox-6,2,12,9); } // de costas: só cabelo
  else {
    g.fillStyle='#20161a';
    if (dir===0){ g.fillRect(ox-3,6,2,2); g.fillRect(ox+1,6,2,2); }
    else if (dir===2){ g.fillRect(ox-4,6,2,2); }
    else if (dir===3){ g.fillRect(ox+2,6,2,2); }
    if (pal.oculos && dir!==1){
      g.strokeStyle='#2a2018'; g.lineWidth=0.8;
      g.strokeRect(ox-4.2,5.4,3,3); g.strokeRect(ox+1.2,5.4,3,3);
      g.beginPath(); g.moveTo(ox-1.2,6.8); g.lineTo(ox+1.2,6.8); g.stroke();
    }
    if (dir===0){ g.strokeStyle='#8a3b32'; g.lineWidth=0.8; g.beginPath(); g.moveTo(ox-2,9.6); g.quadraticCurveTo(ox,10.6,ox+2,9.6); g.stroke(); }
    g.fillStyle='rgba(230,120,110,.35)'; g.fillRect(ox-5,8,2,1.6); g.fillRect(ox+3,8,2,1.6);
  }
}
function sprHeroi(dir,wf,act){
  var k='h'+dir+wf+(act?1:0);
  if (SPR[k]) return SPR[k];
  var c=mk(20,30), g=c.getContext('2d'); g.imageSmoothingEnabled=false;
...
function pintaFace(cv){
  ...
  g.fillStyle='rgba(230,120,110,.35)'; g.fillRect(3,8,2,2); g.fillRect(11,8,2,2);
  ...
  g.strokeStyle='#2a2018'; g.lineWidth=0.7; g.strokeRect(5,6,3,3); g.strokeRect(8,6,3,3);
...
function construirMapas(){
  ...
  Object.keys(MAPAS).forEach(function(k){ prerenderTerreno(MAPAS[k]); });
}
...
function render(){
  ...
  // chão pré-renderizado (textura de grama/pedra + flores/tufos)
  if (!m.terrenoCV) prerenderTerreno(m);
  ctx.drawImage(m.terrenoCV, 0, 0);
  // água
  var tnow=Date.now();
  m.agua.forEach(function(a){
    if (a.x<t0x||a.x>t1x||a.y<t0y||a.y>t1y) return;
    desenhaAgua(a.x*TILE, a.y*TILE, tnow);
  });
  ...
  Object.keys(NPCS).forEach(function(nk){ var s=npcState[nk]; if(s&&s.mapa===E.mapa) draws.push({y:s.y+12, f:function(){ ctx.drawImage(sprNPC(nk,s.dir,s.wf), Math.round(s.x-10), Math.round(s.y-21)); drawNome(NPCS[nk].nome, s.x, s.y-25); }}); });
  draws.push({y:jogador.y+12, f:function(){
    var act = jogador.act>0;
    ctx.drawImage(sprHeroi(jogador.dir, jogador.mv?jogador.wf:0, act), Math.round(jogador.x-10), Math.round(jogador.y-21));
  }});
...
function drawNome(txt,x,y){
  ctx.font='700 6px "Segoe UI"'; ctx.textAlign='center';
  var w2=txt.length*3.6+8;
  rrp(ctx, x-w2/2, y-7, w2, 8, 4);
  ctx.fillStyle='rgba(30,18,12,.72)'; ctx.fill();
...
function drawPredio(p){
  ...
  var wallTop=y+9;
  // sombra no chão
  ctx.fillStyle='rgba(0,0,0,.16)'; ctx.beginPath(); ctx.ellipse(x+w/2, y+h+2, w*0.44, 5, 0,0,7); ctx.fill();
  // parede com tábuas
  ctx.fillStyle=p.cor; ctx.fillRect(x,wallTop,w,h-9);
  for (var i=4;i<w;i+=6){ ctx.fillStyle=((i/6|0)%2)?'rgba(0,0,0,.06)':'rgba(255,255,255,.05)'; ctx.fillRect(x+i,wallTop,3,h-9); }
  ctx.fillStyle='rgba(255,255,255,.14)'; ctx.fillRect(x,wallTop,w,2);
  ctx.fillStyle='rgba(0,0,0,.18)'; ctx.fillRect(x,y+h-4,w,4);
  // telhado (com sombra de um lado + linhas de telha)
  var apexY=y-8;
  ctx.fillStyle=p.teto;
  ctx.beginPath(); ctx.moveTo(x-4,wallTop+2); ctx.lineTo(x+w/2,apexY); ctx.lineTo(x+w+4,wallTop+2); ctx.closePath(); ctx.fill();
  ctx.fillStyle=shade(p.teto,0.68);
  ctx.beginPath(); ctx.moveTo(x+w/2,apexY); ctx.lineTo(x+w+4,wallTop+2); ctx.lineTo(x+w/2,wallTop+2); ctx.closePath(); ctx.fill();
  ctx.strokeStyle='rgba(0,0,0,.16)'; ctx.lineWidth=1;
  for (var ry=wallTop+2; ry>apexY+3; ry-=4){
    var frac=(wallTop+2-ry)/(wallTop+2-apexY), hw=(w/2+4)*(1-frac);
    ctx.beginPath(); ctx.moveTo(x+w/2-hw,ry); ctx.lineTo(x+w/2+hw,ry); ctx.stroke();
  }
  ctx.fillStyle=shade(p.teto,1.35); ctx.fillRect(x+w/2-1,apexY-1,2,4);
  // chaminé + fumaça
  var chx=x+w*0.7;
  ctx.fillStyle=shade(p.teto,0.6); ctx.fillRect(chx,y-4,6,13);
  ctx.fillStyle=shade(p.teto,0.4); ctx.fillRect(chx,y-4,6,2);
  var sm=(Date.now()/900)%3;
  ctx.fillStyle='rgba(255,255,255,'+Math.max(0,0.55-sm*0.14)+')';
  ctx.beginPath(); ctx.arc(chx+3, y-6-sm*5, 2+sm*0.8, 0,7); ctx.fill();
  // porta
  var px=(p.tx+Math.floor(p.tw/2))*TILE;
  ctx.fillStyle='#3a2312'; ctx.fillRect(px-1, y+h-TILE-2, TILE+2, TILE+2);
  ctx.fillStyle='#5c3a20'; ctx.fillRect(px, y+h-TILE, TILE, TILE);
  ctx.fillStyle='rgba(0,0,0,.22)'; ctx.fillRect(px+TILE/2-1, y+h-TILE, 1, TILE);
  ctx.fillStyle='#2a1a0e'; ctx.fillRect(px+2, y+h-9, TILE-4, 5);
  ctx.fillStyle='#e7b84f'; ctx.beginPath(); ctx.arc(px+TILE-4, y+h-6, 1.2, 0,7); ctx.fill();
  ctx.fillStyle='#cdb98f'; ctx.fillRect(px-3, y+h, TILE+6, 3);
  ctx.fillStyle='rgba(0,0,0,.15)'; ctx.fillRect(px-3, y+h+2, TILE+6, 1);
  // janelas com caixilho + floreira
  if (w>=4*TILE){
    [x+7, x+w-15].forEach(function(jx){
      ctx.fillStyle='#3a2312'; ctx.fillRect(jx-1, y+16, 10,10);
      ctx.fillStyle='#bfe3ff'; ctx.fillRect(jx, y+17, 8,8);
      ctx.fillStyle='rgba(255,255,255,.4)'; ctx.fillRect(jx,y+17,3,8);
      ctx.strokeStyle='#3a2312'; ctx.lineWidth=1;
      ctx.beginPath(); ctx.moveTo(jx+4,y+17); ctx.lineTo(jx+4,y+25); ctx.moveTo(jx,y+21); ctx.lineTo(jx+8,y+21); ctx.stroke();
      ctx.fillStyle='#3f7a3a'; ctx.fillRect(jx-2,y+25,12,2);
      ctx.fillStyle='#d84a7a'; ctx.fillRect(jx-1,y+24,2,2); ctx.fillRect(jx+3,y+24,2,2); ctx.fillRect(jx+7,y+24,2,2);
    });
  }
  // placa com o nome
  var nw=p.nome.length*4.6+12;
  rrp(ctx, x+w/2-nw/2, y+1, nw, 10, 4);
  ctx.fillStyle='rgba(28,17,11,.88)'; ctx.fill();
  ctx.strokeStyle='rgba(231,184,79,.6)'; ctx.lineWidth=1; ctx.stroke();
  ctx.fillStyle='#ffe0a0'; ctx.font='700 6px "Segoe UI"'; ctx.textAlign='center';
  ctx.fillText(p.nome, x+w/2, y+7.5); ctx.textAlign='left';
}
function drawArvore(a){
  var v=((a.x|0)*31 + (a.y|0)*17) % 3;
  var copaCor = v===0 ? '#3f7a3a' : v===1 ? '#3a8552' : '#4a7a3a';
  ctx.fillStyle='rgba(0,0,0,.2)'; ctx.beginPath(); ctx.ellipse(a.x,a.y+1,8,3,0,0,7); ctx.fill();
  ctx.fillStyle='#5a3d24'; ctx.fillRect(a.x-2,a.y-13,4,13);
  ctx.fillStyle='rgba(0,0,0,.2)'; ctx.fillRect(a.x,a.y-13,2,13);
  ctx.fillStyle=shade(copaCor,0.8);
  ctx.beginPath(); ctx.arc(a.x,a.y-19,10,0,7); ctx.fill();
  ctx.beginPath(); ctx.arc(a.x-8,a.y-13,7.5,0,7); ctx.fill();
  ctx.beginPath(); ctx.arc(a.x+8,a.y-13,7.5,0,7); ctx.fill();
  ctx.fillStyle=copaCor;
  ctx.beginPath(); ctx.arc(a.x-1,a.y-20,9,0,7); ctx.fill();
  ctx.beginPath(); ctx.arc(a.x-8,a.y-14,6,0,7); ctx.fill();
  ctx.beginPath(); ctx.arc(a.x+7,a.y-14,6,0,7); ctx.fill();
  ctx.fillStyle=shade(copaCor,1.35);
  ctx.beginPath(); ctx.arc(a.x-3,a.y-24,5.5,0,7); ctx.fill();
  ctx.beginPath(); ctx.arc(a.x-9,a.y-16,3,0,7); ctx.fill();
  if (v===2){ ctx.fillStyle='#c0392b'; [[-4,-18],[5,-21],[1,-14]].forEach(function(o){ ctx.beginPath(); ctx.arc(a.x+o[0],a.y+o[1],1.3,0,7); ctx.fill(); }); }
}
function drawProp(p){
  var x=p.x, y=p.y;
  if (p.tipo==='correio'){
    ctx.fillStyle='rgba(0,0,0,.15)'; ctx.beginPath(); ctx.ellipse(x,y+2,4,1.6,0,0,7); ctx.fill();
    ctx.fillStyle='#6a4a28'; ctx.fillRect(x-1,y-2,2,9);
    ctx.fillStyle='#3a6a9a'; rrp(ctx,x-5,y-11,10,8,3); ctx.fill();
    ctx.fillStyle='rgba(255,255,255,.25)'; ctx.fillRect(x-4,y-10,3,6);
    ctx.fillStyle='#e7b84f'; ctx.fillRect(x+2,y-8,2,2);
  } else if (p.tipo==='remessa'){
    ctx.fillStyle='rgba(0,0,0,.16)'; ctx.beginPath(); ctx.ellipse(x,y+3,9,2.4,0,0,7); ctx.fill();
    ctx.fillStyle='#8a5a34'; ctx.fillRect(x-8,y-10,16,12);
    ctx.fillStyle=shade('#8a5a34',0.75); ctx.fillRect(x-8,y-2,16,4);
    ctx.fillStyle='#a9723f'; ctx.fillRect(x-8,y-10,16,3);
    ctx.strokeStyle='#5a3a22'; ctx.lineWidth=1.4; ctx.strokeRect(x-8.5,y-10.5,17,12);
    ctx.fillStyle='#e7b84f'; ctx.fillRect(x-1,y-7,2,3);
  } else if (p.tipo==='pc'){
    ctx.fillStyle='#8a5a3a'; ctx.fillRect(x-8,y-2,16,3);
    ctx.fillStyle='#3a3a4a'; rrp(ctx,x-6,y-11,12,9,2); ctx.fill();
    ctx.fillStyle='#7ec6ff'; ctx.fillRect(x-4,y-9,8,5);
    ctx.fillStyle='rgba(255,255,255,.35)'; ctx.fillRect(x-4,y-9,2,5);
    ctx.fillStyle='#26262f'; ctx.fillRect(x-2,y-2,4,2);
    ctx.fillStyle='#555'; ctx.fillRect(x-5,y-0,10,2);
  } else if (p.tipo==='quadro'){
    ctx.fillStyle='#5a3a22'; rrp(ctx,x-9,y-13,18,13,2); ctx.fill();
    ctx.fillStyle='#e8e2d0'; ctx.fillRect(x-7,y-11,14,9);
    ctx.fillStyle='#b8362e'; ctx.fillRect(x-5,y-9,5,3); ctx.fillStyle='#fff'; ctx.fillRect(x-5.5,y-9.5,1,4);
    ctx.fillStyle='#3a6a9a'; ctx.fillRect(x+1,y-5,5,2); ctx.fillStyle='#fff'; ctx.fillRect(x+0.5,y-5.5,1,3);
    ctx.fillStyle='#e7b84f'; ctx.fillRect(x-6,y-6,4,3);
  } else if (p.tipo==='mesa'){
    ctx.fillStyle='rgba(0,0,0,.14)'; ctx.beginPath(); ctx.ellipse(x,y+5,10,2.4,0,0,7); ctx.fill();
    ctx.fillStyle='#6a4326'; ctx.fillRect(x-8,y+1,2,4); ctx.fillRect(x+6,y+1,2,4);
    ctx.fillStyle='#9a6a3f'; ctx.fillRect(x-9,y-4,18,6);
    ctx.fillStyle='rgba(255,255,255,.18)'; ctx.fillRect(x-9,y-4,18,2);
    ctx.fillStyle='#faf4e6'; ctx.fillRect(x-3,y-7,7,3);
    ctx.strokeStyle='#c9a27a'; ctx.lineWidth=1; ctx.strokeRect(x-3.5,y-7.5,8,4);
  } else if (p.tipo==='cafeteira'){
    ctx.fillStyle='#6d4c41'; rrp(ctx,x-4,y-9,8,10,2); ctx.fill();
    ctx.fillStyle='#c9a27a'; ctx.fillRect(x-4,y-9,8,2);
    ctx.fillStyle='#3a2a20'; ctx.fillRect(x+3,y-6,3,4);
    ctx.fillStyle='rgba(255,255,255,.4)'; ctx.fillRect(x-3,y-7,2,6);
    var sm2=(Date.now()/800)%2;
    ctx.fillStyle='rgba(255,255,255,'+(0.5-sm2*0.2)+')'; ctx.beginPath(); ctx.arc(x-1,y-11-sm2*3,1.4,0,7); ctx.fill();
  } else if (p.tipo==='gaveteiro'){
    ctx.fillStyle='rgba(0,0,0,.15)'; ctx.beginPath(); ctx.ellipse(x,y+2,7,2,0,0,7); ctx.fill();
    ctx.fillStyle=p.cor; ctx.fillRect(x-6,y-12,12,14);
    ctx.fillStyle='rgba(255,255,255,.12)'; ctx.fillRect(x-6,y-12,3,14);
    ctx.fillStyle='rgba(0,0,0,.22)'; ctx.fillRect(x-6,y-6,12,1); ctx.fillRect(x-6,y-1,12,1);
    ctx.fillStyle='#3a3a3a'; ctx.fillRect(x-1,y-9,2,1); ctx.fillRect(x-1,y-4,2,1);
  } else if (p.tipo==='fonte'){
    ctx.fillStyle='rgba(0,0,0,.15)'; ctx.beginPath(); ctx.ellipse(x,y+2,13,3,0,0,7); ctx.fill();
    ctx.fillStyle='#9aa0a8'; ctx.beginPath(); ctx.arc(x,y,11,0,7); ctx.fill();
    ctx.fillStyle=shade('#9aa0a8',1.2); ctx.beginPath(); ctx.arc(x,y-1,11,Math.PI,Math.PI*2); ctx.fill();
    var wob=Math.sin(Date.now()/300)*1;
    ctx.fillStyle='#4f92c0'; ctx.beginPath(); ctx.arc(x,y,8,0,7); ctx.fill();
    ctx.fillStyle='#6aa8d8'; ctx.beginPath(); ctx.arc(x,y-1+wob*0.3,8,Math.PI,Math.PI*2); ctx.fill();
    ctx.fillStyle='rgba(255,255,255,.5)'; ctx.beginPath(); ctx.arc(x-3,y-2,1.4,0,7); ctx.fill();
    ctx.fillStyle='#9aa0a8'; ctx.fillRect(x-2,y-13,4,12);
    ctx.fillStyle='rgba(255,255,255,.55)'; ctx.beginPath(); ctx.arc(x,y-14-wob,2,0,7); ctx.fill();
  } else if (p.tipo==='placa'){
    ctx.fillStyle='rgba(0,0,0,.14)'; ctx.beginPath(); ctx.ellipse(x,y+2,4,1.4,0,0,7); ctx.fill();
    ctx.fillStyle='#7a5a3a'; ctx.fillRect(x-1,y-2,2,8);
    ctx.fillStyle=p.cor; rrp(ctx,x-9,y-12,18,9,2); ctx.fill();
    ctx.strokeStyle='rgba(0,0,0,.25)'; ctx.lineWidth=1; ctx.stroke();
    ctx.fillStyle='#fff'; ctx.font='700 5px "Segoe UI"'; ctx.textAlign='center'; ctx.fillText('FÓRUM', x, y-6); ctx.textAlign='left';
  } else if (p.tipo==='doca'){
    ctx.fillStyle='rgba(0,0,0,.15)'; ctx.fillRect(x-15,y+4,30,3);
    ctx.fillStyle='#8a6644'; ctx.fillRect(x-14,y-3,28,8);
    for (var pl2=-12; pl2<14; pl2+=6){ ctx.fillStyle='rgba(0,0,0,.14)'; ctx.fillRect(x+pl2,y-3,1,8); }
    ctx.fillStyle='#6a4326'; for(var di=-12;di<14;di+=6) ctx.fillRect(x+di,y+5,3,6);
  } else if (p.tipo==='toco'){
    ctx.fillStyle='rgba(0,0,0,.16)'; ctx.beginPath(); ctx.ellipse(x,y,6,2,0,0,7); ctx.fill();
    ctx.fillStyle='#6a4a28'; ctx.fillRect(x-5,y-6,10,7);
    ctx.fillStyle='#9a7a4a'; ctx.beginPath(); ctx.ellipse(x,y-6,5,2.6,0,0,7); ctx.fill();
    ctx.strokeStyle='rgba(0,0,0,.2)'; ctx.lineWidth=0.8; ctx.beginPath(); ctx.ellipse(x,y-6,3,1.6,0,0,7); ctx.stroke();
...
function drawPlanta(tx,ty,pl){
  ...
  ctx.fillStyle='rgba(0,0,0,.12)'; ctx.beginPath(); ctx.ellipse(x,y+1,4,1.4,0,0,7); ctx.fill();
  ctx.fillStyle='#2f6a2f'; ctx.fillRect(x-1, y-Math.max(2,f*11), 2, Math.max(2,f*11));
  if (pl.estagio>=C.dias){
    var bob=Math.sin(Date.now()/260)*1;
    ctx.fillStyle=shade(mix('#8a6a44',C.cor,1),0.8); rrp(ctx,x-5,y-13+bob,10,10,2); ctx.fill();
    ctx.fillStyle=mix(C.cor,'#ffffff',0.18); rrp(ctx,x-5,y-13+bob,10,4,2); ctx.fill();
    ctx.strokeStyle='rgba(255,255,255,.7)'; ctx.lineWidth=1; ctx.strokeRect(x-6,y-14+bob,12,12);
    ctx.fillStyle='rgba(255,255,255,'+(0.4+Math.sin(Date.now()/180)*0.3)+')';
    ctx.beginPath(); ctx.arc(x+5,y-16+bob,1.3,0,7); ctx.fill();
  } else {
    var leafCor = mix('#3f7a3a',C.cor,0.15);
    ctx.fillStyle=shade(leafCor,0.85); ctx.beginPath(); ctx.arc(x-2, y-f*11, 2+f*2.4, 0,7); ctx.fill();
    ctx.fillStyle=leafCor; ctx.beginPath(); ctx.arc(x+2, y-f*9, 2+f*2.2, 0,7); ctx.fill();
    ctx.fillStyle=shade(leafCor,1.25); ctx.beginPath(); ctx.arc(x, y-f*13, 1.4+f*1.4, 0,7); ctx.fill();
    if (pl.prot){ ctx.fillStyle='rgba(120,180,255,.32)'; rrp(ctx,tx*TILE+1,ty*TILE+1,TILE-2,TILE-2,3); ctx.fill(); }
  }
}
function drawEgg(eg){
  var x=eg.tx*TILE+8, y=eg.ty*TILE+14, t=Date.now()/400, bob=Math.sin(t)*1.4;
  ctx.fillStyle='rgba(0,0,0,.18)'; ctx.beginPath(); ctx.ellipse(x,y+1,5.5,2,0,0,7); ctx.fill();
  // halo suave
  var gr=ctx.createRadialGradient(x,y-8+bob,0,x,y-8+bob,11);
  gr.addColorStop(0, mix(eg.cor,'#ffffff',0.5).replace('rgb','rgba').replace(')',',.35)'));
  gr.addColorStop(1, 'rgba(255,255,255,0)');
  ctx.fillStyle=gr; ctx.beginPath(); ctx.arc(x,y-8+bob,11,0,7); ctx.fill();
  ctx.save(); ctx.translate(0,bob);
  ctx.fillStyle=eg.cor;
  ctx.strokeStyle='rgba(0,0,0,.28)'; ctx.lineWidth=0.8;
  if (eg.k==='livro'){ ctx.fillRect(x-5,y-8,10,9); ctx.strokeRect(x-5,y-8,10,9); ctx.fillStyle='rgba(255,255,255,.8)'; ctx.fillRect(x-1,y-8,2,9); ctx.fillStyle=shade(eg.cor,0.7); ctx.fillRect(x-5,y-8,2,9); }
  else if (eg.k==='coluna'){ ctx.fillRect(x-3,y-14,6,14); ctx.fillRect(x-5,y-15,10,2); ctx.fillRect(x-5,y-1,10,2); ctx.fillStyle='rgba(255,255,255,.3)'; ctx.fillRect(x-3,y-14,2,14); }
  else if (eg.k==='ampulheta'){ ctx.beginPath(); ctx.moveTo(x-4,y-12); ctx.lineTo(x+4,y-12); ctx.lineTo(x,y-6); ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.beginPath(); ctx.moveTo(x-4,y); ctx.lineTo(x+4,y); ctx.lineTo(x,y-6); ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.fillStyle='#e7b84f'; ctx.fillRect(x-5,y-13,10,1.4); ctx.fillRect(x-5,y-0.4,10,1.4); }
  else if (eg.k==='selo'){ ctx.beginPath(); ctx.arc(x,y-5,5,0,7); ctx.fill(); ctx.stroke(); ctx.fillStyle='#e7b84f'; ctx.beginPath(); ctx.arc(x,y-5,2,0,7); ctx.fill(); ctx.strokeStyle='rgba(255,255,255,.5)'; ctx.beginPath(); ctx.arc(x-1.5,y-6.5,1.4,0,7); ctx.stroke(); }
  else if (eg.k==='carimbo'){ ctx.fillRect(x-2,y-12,4,6); ctx.fillRect(x-5,y-6,10,6); ctx.strokeRect(x-5,y-6,10,6); ctx.fillStyle='rgba(255,255,255,.25)'; ctx.fillRect(x-5,y-6,3,6); }
  else if (eg.k==='pergaminho'){ ctx.fillRect(x-6,y-6,12,5); ctx.fillStyle='#c99a34'; ctx.fillRect(x-7,y-7,2,7); ctx.fillRect(x+5,y-7,2,7); ctx.fillStyle='rgba(255,255,255,.3)'; ctx.fillRect(x-6,y-6,12,1.4); }
  else if (eg.k==='cadeira'){ ctx.fillRect(x-4,y-4,8,3); ctx.fillRect(x-4,y-11,8,7); ctx.fillRect(x-4,y-1,2,3); ctx.fillRect(x+2,y-1,2,3); ctx.fillStyle='rgba(255,255,255,.2)'; ctx.fillRect(x-4,y-11,2,7); }
  else if (eg.k==='placa'){ ctx.fillRect(x-1,y-2,2,6); ctx.fillRect(x-6,y-11,12,8); ctx.strokeRect(x-6,y-11,12,8); }
  else if (eg.k==='caneca'){ ctx.fillRect(x-4,y-8,8,9); ctx.strokeStyle=shade(eg.cor,0.6); ctx.strokeRect(x+3,y-6,3,4); ctx.fillStyle='rgba(255,255,255,.4)'; ctx.fillRect(x-3,y-7,2,7); ctx.fillStyle='#5a3418'; ctx.fillRect(x-3,y-8,6,1.4); }
  else { ctx.fillRect(x-4,y-8,8,8); ctx.strokeRect(x-4,y-8,8,8); }
  ctx.restore();
  // brilho girando (losango dourado)
  ctx.save(); ctx.translate(x, y-17+Math.sin(t*1.15)*1.6); ctx.rotate(t*1.3);
  ctx.fillStyle='#ffe08a'; ctx.beginPath(); ctx.moveTo(0,-3.4); ctx.lineTo(3.4,0); ctx.lineTo(0,3.4); ctx.lineTo(-3.4,0); ctx.closePath(); ctx.fill();
  ctx.fillStyle='rgba(255,255,255,.85)'; ctx.beginPath(); ctx.moveTo(0,-1.6); ctx.lineTo(1.6,0); ctx.lineTo(0,1.6); ctx.lineTo(-1.6,0); ctx.closePath(); ctx.fill();
  ctx.restore();
}
function drawMarcador(A,i){
  ...
  if (lib && !feita){
    var gr=ctx.createRadialGradient(x,y+2,0,x,y+2,13);
    gr.addColorStop(0,'rgba(231,184,79,'+(0.4+Math.sin(t)*0.14)+')'); gr.addColorStop(1,'rgba(231,184,79,0)');
    ctx.fillStyle=gr; ctx.beginPath(); ctx.arc(x,y+2,13,0,7); ctx.fill();
  }
  ctx.fillStyle = feita ? 'rgba(76,154,104,.4)' : lib ? 'rgba(231,184,79,.4)' : 'rgba(90,90,90,.22)';
  ctx.beginPath(); ctx.ellipse(x,y+4,10,4,0,0,7); ctx.fill();
  // placa de madeira num pilar
  var by=y-2-Math.max(0,Math.sin(t)*(lib&&!feita?1.6:0));
  ctx.fillStyle='#6a4a28'; ctx.fillRect(x-1,by+2,2,8);
  ctx.fillStyle=feita?'#4c9a68':(lib?'#e7b84f':'#8a8378');
  rrp(ctx,x-8,by-8,16,10,3); ctx.fill();
  ctx.strokeStyle='rgba(0,0,0,.25)'; ctx.lineWidth=1; ctx.stroke();
  ctx.fillStyle='rgba(255,255,255,.25)'; ctx.fillRect(x-7,by-7,14,2);
  ctx.font='8px "Segoe UI"'; ctx.textAlign='center';
  ctx.fillStyle='#fff';
  ctx.fillText(feita?'✓':(lib?A.emoji:'🔒'), x, by-2);
  ctx.textAlign='left';
}
function drawPesca(){
  var s=pescaSt;
